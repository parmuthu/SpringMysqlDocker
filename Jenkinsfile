pipeline {
	...
	stages {
       ...
       steps
	    ...
	    {
	    checkout changelog: false, poll: false, scm: scmGit(branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[credentialsId: '552894fe-a066-4c30-8dda-413876e0f0af', url: 'https://github.com/parmuthu/SpringMysqlDocker.git']])
	    }
       }
		stage('Build project'){
			steps 
	    ...
	    stage('Build docker image') {
			steps {
			  ...
			  script
			  {
			      bat 'docker build -t parmv/productapp .'
			  }
			}
		}
